...
async function register() {
    ...
    if (username) {
        const apiRegOptsResp = await fetch('/lab1/registration_start', {
            method: 'POST',
        ...
        const apiRegVerResp = await fetch('/lab1/registration_verify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              username: username,
...
async function authenticate() {
    const username = document.getElementById("username").value.trim();
    if (username) {
        const apiAuthOptsResp = await fetch('/lab1/authentication_start', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                username: username
        ...
        const apiAuthVerResp = await fetch('/lab1/authentication_verify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              username: username,
